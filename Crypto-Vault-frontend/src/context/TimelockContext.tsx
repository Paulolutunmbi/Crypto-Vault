...
export const TimelockProvider: React.FC<{ children: ReactNode }> = ({ children }) => {
  ...
  const [tokens, setTokens] = useState<TokenInfo[]>([tokenTemplate]);
  ...
  const refreshChainState = async (address: string, chainId = SEPOLIA_CHAIN_ID) => {
    ...
    if (!provider || chainId !== SEPOLIA_CHAIN_ID || !MOCK_TOKEN_ADDRESS || !TOKEN_LOCKER_ADDRESS) return;

    const ethBalance = Number(ethers.formatEther(await provider.getBalance(address)));
    const token = new Contract(MOCK_TOKEN_ADDRESS, ERC20_ABI, provider);
    const balance = Number(ethers.formatUnits(await token.balanceOf(address), tokenTemplate.decimals));
    ...
    const loaded = await Promise.all(
      ids.map(async id => {
        ...
        return {
          ...
          tokenSymbol: symbol,
          tokenName: name,
          amount: Number(ethers.formatUnits(item.amount, decimals)),
          amountUsd: 0,
    ...
    setLocks(loaded);
    setTokens([{ ...tokenTemplate, address: MOCK_TOKEN_ADDRESS, userBalance: balance }]);
  };
  ...
  const createLock = async ({ tokenSymbol, amount, unlockTimestamp }: CreateLockParams) => {
    if (!wallet.isConnected || wallet.network.chainId !== SEPOLIA_CHAIN_ID) throw new Error('wrong network');
    if (tokenSymbol !== 'MTK') throw new Error('invalid token');
    ...
    const token = new Contract(MOCK_TOKEN_ADDRESS, ERC20_ABI, signer);
    const locker = new Contract(TOKEN_LOCKER_ADDRESS, TOKEN_LOCKER_ABI, signer);
    const rawAmount = ethers.parseUnits(amount.toString(), tokenTemplate.decimals);

    if ((await token.balanceOf(wallet.address)) < rawAmount) throw new Error('insufficient MTK balance');
    if ((await token.allowance(wallet.address, TOKEN_LOCKER_ADDRESS)) < rawAmount) {
      setTxState({ step: 'approving', title: 'Approve MTK', description: 'Waiting for wallet confirmation...' });
      const approval = await token.approve(TOKEN_LOCKER_ADDRESS, rawAmount);
      setTxState({ step: 'approving', title: 'Approve MTK', description: 'Transaction pending...', txHash: approval.hash });
      await approval.wait();
    }
    ...
    const transaction = await locker.createLock(MOCK_TOKEN_ADDRESS, rawAmount, Math.floor(unlockTimestamp / 1000), { value: PROTOCOL_FEE });
    ...
    addToast({ type: 'success', title: 'Time-Lock Created', message: `${amount} MTK locked until ${new Date(unlockTimestamp).toLocaleString()}.` });
    return created;
  };
  ...
  const claimFaucet = async () => {
    ...
    const nextBalance = Number(ethers.formatUnits(await token.balanceOf(wallet.address), tokenTemplate.decimals));
    setTokens(prev => prev.map(item => (item.symbol === 'MTK' ? { ...item, userBalance: nextBalance } : item)));
    await refreshChainState(wallet.address);
  ...
  const withdrawLock = async (lockId: string) => {
    ...
    if (!lock || lock.owner.toLowerCase() !== wallet.address.toLowerCase() || lock.withdrawn || lock.unlocksAtTimestamp > Date.now()) throw new Error('withdrawal unavailable');
